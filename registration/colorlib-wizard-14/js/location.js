...
// Fetch regions on page load for Step 4
fetch(regionUrl)
    .then(response => {
        if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
        return response.json();
    ...
    .catch(error => console.error('Error fetching regions for Step 4:', error));

// Fetch provinces when a region is selected in Step 4
step4RegionSelect.addEventListener('change', function() {
    ...
    if (selectedRegion) {
        ...
        fetch(provinceUrl)
            ...
            .catch(error => console.error('Error fetching provinces for Step 4:', error));
    }
});

// Fetch cities when a province is selected in Step 4
step4ProvinceSelect.addEventListener('change', function() {
    ...
    if (selectedProvince) {
        ...
        fetch(citiesUrl)
            ...
            .catch(error => console.error('Error fetching cities for Step 4:', error));
    }
});

// Fetch barangays when a city is selected in Step 4
step4CitySelect.addEventListener('change', function() {
    ...
    if (selectedCity) {
        ...
        fetch(barangayUrl)
            ...
            .catch(error => console.error('Error fetching barangays for Step 4:', error));
    }
});

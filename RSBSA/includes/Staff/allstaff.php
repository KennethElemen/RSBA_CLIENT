...
$query = "SELECT 
            ...
            ua.role,  -- Include the staff role here
        ...
        GROUP BY 
            ua.user_id, ua.email, ua.accountStatus, ua.account_id, u.first_name, u.middle_name, u.sur_name, u.date_of_birth, u.profile_picture, ua.role  -- Add staff_role here
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<td class="cell">' . htmlspecialchars($row['first_name'] . ' ' . $row['middle_name'] . ' ' . $row['sur_name']) . '</td>';
        ...
        // Display the staff role
        echo '<td class="cell">' . htmlspecialchars($row['role']) . '</td>';  // Add this line to display staff role
        ...
        echo '<td>
                <button type="button" class="btn btn-primary btn-sm px-2"
                    ...
                    data-job-role="' . htmlspecialchars($row['job_role']) . '"
    ...
    echo '<tr><td colspan="10" class="text-center">No users found</td></tr>';  // Update colspan if needed
}

$totalResult = $dbConnection->query("SELECT COUNT(*) as total FROM useraccounts WHERE accountStatus IN ('accepted', 'Pending')");
